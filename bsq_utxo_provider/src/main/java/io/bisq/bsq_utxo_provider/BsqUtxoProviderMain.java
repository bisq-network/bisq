...
public class BsqUtxoProviderMain extends BisqExecutable {
    private static final Logger log = LoggerFactory.getLogger(BsqUtxoProviderMain.class);
    private static final long MAX_MEMORY_MB_DEFAULT = 400;
    private static final long CHECK_MEMORY_PERIOD_SEC = 3 * 60;
    private BsqUtxoProvider statistics;
    private volatile boolean stopped;
    private static long maxMemory = MAX_MEMORY_MB_DEFAULT;

    public static void main(String[] args) throws Exception {
        final ThreadFactory threadFactory = new ThreadFactoryBuilder()
                .setNameFormat("UtxoProvider")
                .setDaemon(true)
                .build();
        ...
        Thread.currentThread().setContextClassLoader(BsqUtxoProviderMain.class.getClassLoader());

        new BsqUtxoProviderMain().execute(args);
    }

    @Override
    protected void doExecute(OptionSet options) {
        final BisqEnvironment environment = getBisqEnvironment(options);
        BsqUtxoProvider.setEnvironment(environment);

        UserThread.execute(() -> statistics = new BsqUtxoProvider());
