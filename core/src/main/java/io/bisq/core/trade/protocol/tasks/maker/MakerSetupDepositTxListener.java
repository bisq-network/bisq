...
public class MakerSetupDepositTxListener extends TradeTask {
        ...
        try {
            ...
            if (trade.getState().getPhase() == Trade.Phase.TAKER_FEE_PUBLISHED) {
                ...
                if (trade.getDepositTx() != null) {
                    ...
                    log.warn("Deposit tx is null. We move it to failed trades. Trade ID=" + trade.getId());
                    UserThread.execute(() -> processModel.getTradeManager().addTradeToFailedTrades(trade));
                }
            }
    ...
    private void swapReservedForTradeEntry() {
        log.debug("swapReservedForTradeEntry, offerId={}, RESERVED_FOR_TRADE", trade.getId());
