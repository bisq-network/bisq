...
public class TradeWalletService {
    ...
    /**
     * Maker creates atomic transaction and signs their inputs.
     *
     * Tx output format:
    ...
    public Transaction makerCreatesAndSignsAtomicTx(Coin makerBsqPayout,
        ...
        if (makerBsqPayout.isPositive())
            atomicTx.addOutput(makerBsqPayout, Address.fromBase58(params, makerBsqAddress));
        if (takerBsqPayout.isPositive())
            atomicTx.addOutput(takerBsqPayout, Address.fromBase58(params, takerBsqAddress));
        if (takerBtcPayout.isPositive())
            atomicTx.addOutput(takerBtcPayout, Address.fromBase58(params, takerBtcAddress));
        if (makerBtcPayout.isPositive())
            atomicTx.addOutput(makerBtcPayout, Address.fromBase58(params, makerBtcAddress));
        if (btcTradeFee.isPositive())
            atomicTx.addOutput(btcTradeFee, Address.fromBase58(params, btcTradeFeeAddress));
    ...
    public List<TransactionInput> getInputsForAddress(Address address, Coin amount) throws WalletException {
        Transaction dummyTx = new Transaction(params);
        dummyTx.addOutput(new TransactionOutput(params, dummyTx, amount, new ECKey().toAddress(params)));
