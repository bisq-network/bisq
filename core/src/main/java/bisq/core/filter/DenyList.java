...
public class DenyList {
    ...
    public DenyList(Config config) {
        this(config.ignoreDenyList ?
                new Properties() :
                loadProperties(resolveResourceName(config), !config.denyListResource.isEmpty()));
    ...
    @VisibleForTesting
    static String resolveResourceName(Config config) {
        return config.denyListResource.isEmpty() ? resourceName(config) : config.denyListResource;
    }

    private static Properties loadProperties(String resourceName, boolean failIfMissing) {
        Properties properties = new Properties();
        if (resourceName == null || resourceName.isEmpty()) {
            log.info("No DenyList resource configured");
            return properties;
        }
        ...
        try (InputStream inputStream = DenyList.class.getResourceAsStream("/" + resourceName)) {
            if (inputStream == null) {
                if (failIfMissing) {
                    throw new IllegalArgumentException("DenyList resource not found: " + resourceName);
                }
    ...
    private static List<String> readNodeAddressList(Properties properties, String key) {
        ...
        values.forEach(value -> {
            try {
                ...
                throw new IllegalArgumentException("Invalid DenyList node address in " + key + ": " + value, e);
            }
        });
        return values;
    }
