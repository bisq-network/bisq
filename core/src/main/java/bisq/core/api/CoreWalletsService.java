...
class CoreWalletsService {
    ...
    void verifyWalletsAreAvailable() {
        verifyWalletAndNetworkAreInitialized();
    ...
    void verifyWalletAndNetworkAreInitialized() {
    ...
    private void maybeInitWallet() {
        ...
        // on startup.  This method will set the wallet service's aesKey if necessary,
        // then update balances.
        log.info("Init wallet");
        if (tempAesKey == null)
            throw new IllegalStateException("cannot init encrypted wallet with null key");
        ...
        if (btcWalletService.getAesKey() == null || bsqWalletService.getAesKey() == null) {
            ...
            balances.onAllServicesInitialized();            // btc specific
            // TODO Init BSQ wallet.  How?
        }
    }
