...
public final class Role implements PersistablePayload, NetworkPayload, BondedAsset, ImmutableDaoStateModel, Canonical {
    ...
    private Role(String uid,
        ...
        hash = Hash.getSha256Ripemd160hash(encodeCanonical());
    }
    ...
    public static final CanonicalSchema<Role> SCHEMA = CanonicalSchema.<Role>newBuilder()
            .string(1, Role::getUid)
            .string(2, Role::getName)
            .string(3, Role::getLink)
            .string(4, role -> role.bondedRoleType.name())
            .build();
