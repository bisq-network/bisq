...
@Slf4j
public abstract class AtomicSetupTxListener extends TradeTask {
    ...
    public AtomicSetupTxListener(TaskRunner taskHandler, Trade trade) {
    ...
    private void applyConfidence(TransactionConfidence confidence) {
        ...
        if (processModel.getAtomicModel().getAtomicTx() != null) {
            trade.setAtomicTxId(walletTx.getHashAsString());
            WalletService.printTx("atomicTx received from network", walletTx);
            setState();
        } else {
            log.info("We had the atomic tx already set. tradeId={}, state={}", trade.getId(), trade.getState());
        }
    ...
    protected void setState() {
        ...
        processModel.getTradeManager().addTradeToClosedTrades(trade);
    }
