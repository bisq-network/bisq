...
hash migration: the legacy Java 11 `HashMap` iteration order that the canonical
encoder must mimic for DAO state maps.

## Files

- `HashMapOrderPathProbe.java` compares selected small key vectors across
  `HashMap(Map)`, `putAll`, `Collectors.toMap`, and
  `LegacyHashMapOrderMapEntryIterator`.
...
- `compare-hashmap-order-jdks.sh` runs the probe under Java 11 and Java 21. It
  ...
  `LegacyHashMapOrderMapEntryIterator`, or if either output differs across JDKs.
