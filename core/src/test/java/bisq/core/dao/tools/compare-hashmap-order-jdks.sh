...

{
  ...
  echo "$ROOT_DIR/core/src/main/java/bisq/core/encoding/canonical/LegacyHashMapOrderMapEntryIterator.java"
...
if grep -q '\.collector\.matchesLegacyIterator=false' "$OUT_DIR/hashmap-order-java11.comparable.txt"; then
  echo "Java 11 Collectors.toMap HashMap order does not match LegacyHashMapOrderMapEntryIterator." >&2
  exit 2
fi

if grep -q '\.collector\.matchesLegacyIterator=false' "$OUT_DIR/hashmap-order-java21.comparable.txt"; then
  echo "Java 21 Collectors.toMap HashMap order does not match LegacyHashMapOrderMapEntryIterator." >&2
  exit 2
fi
...
if [[ $ITERATOR_STATUS -ne 0 ]]; then
  echo "LegacyHashMapOrderMapEntryIterator output differs across Java 11 and Java 21. See $ITERATOR_DIFF" >&2
  exit 2
fi
...
echo "Collectors.toMap HashMap order matches LegacyHashMapOrderMapEntryIterator on Java 11 and Java 21."
