...
Here is an overview:
 ...
 - `--allowMainnetSignedWitnessesWithDevPrivilegeKeys`: With `--useDevPrivilegeKeys=true`, accepts legacy mainnet arbitrator roots for signed-witness validation in addition to the active dev key. Default `false`; enable only when testing copied mainnet signed-witness data.
