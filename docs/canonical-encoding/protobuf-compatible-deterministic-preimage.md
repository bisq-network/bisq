...
| Packed versus unpacked repeated numeric fields | Packed repeated numeric fields are unsupported until a Bisq schema needs them. When added, the schema must choose exactly one canonical form; proto3 packed encoding should be the default for new non-legacy numeric repeated fields. |
