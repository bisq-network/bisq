...
| Developer filter signature | `FilterManager` signs `Sha256Hash.of(filter.serializeForHash())` | `FilterManager`, `Filter`, `PaymentAccountFilter` | Not migrated in this change. `Filter` contains `double` and packed repeated integer fields and needs dedicated parity vectors before switching signatures. |
...
| Temp proposal protected payload | P2P storage hashes `TempProposalPayload.serializeForHash()` through protected storage | `TempProposalPayload`, `Proposal`, `P2PDataStorage` | Not migrated in this change except for nested `Proposal` canonical bytes where callers hash the proposal directly. Full protected-storage compatibility still needs a wrapper schema. |
...
or added schemas and focused parity tests. The larger P2P protected-payload,
offer, filter, and payment-account hierarchies remain documented follow-up work
because they need a broader schema and compatibility surface.
