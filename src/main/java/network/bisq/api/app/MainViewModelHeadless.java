 ...
 */
@Slf4j
public class MainViewModelHeadless {
...
//    private TorNetworkSettingsWindow torNetworkSettingsWindow;
    ...
    @Inject
    public MainViewModelHeadless(WalletsManager walletsManager, WalletsSetup walletsSetup,
...
//        this.daoManager = daoManager;
    ...
    private void showTorNetworkSettingsWindow() { // TODO can't show this
        log.error("p2p network not ready"); // TODO how to handle p2p not being ready?
        //        MainView.blur();
                    ...
                    if (exception == null) {
                        ...
                        if (exception instanceof TimeoutException) {
                            ...
                            if (exception.getCause().getCause() instanceof ChainFileLockedException) {
                                log.warn(Res.get("popup.warning.startupFailed.twoInstances"));
                            } else {
                                log.warn(Res.get("error.spvFileCorrupted", exception.getMessage()));
                            }
                        } else {
        ...
        Thread checkIfLocalHostNodeIsRunningThread = new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    UserThread.execute(MainViewModelHeadless.this::readMapsFromResources);
                } finally {
                    if (socket != null) {
                        try {
                            socket.close();
